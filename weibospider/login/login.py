...
def get_redirect(name, data, post_url, session):
    ...
    if 'retcode=101' in login_loop:
        crawler_logger.error('invalid password for {}, please ensure your account and password'.format(name))
        LoginInfoOper.freeze_account(name, 2)
        return ''

    if 'retcode=2070' in login_loop:
        crawler_logger.error('invalid verification code')
        return 'pinerror'

    if 'retcode=4049' in login_loop:
        crawler_logger.warning('account {} need verification for login'.format(name))
