...
def get_reposts(url, session, headers):
    ...
    try:
        print('根微博地址为：{url}'.format(url=url))
        html = session.get(url, headers=headers, timeout=120).text
        time.sleep(60)
    except TimeoutError:
        logging.info('抓取微博{url}超时'.format(url=url))
    else:
        if not status_parse.is_404(html):
            ...
            try:
                html = session.get(root_url, headers=headers, timeout=120).text
                time.sleep(60)
            except TimeoutError:
                logging.info('抓取微博{url}超时'.format(url=root_url))
            else:
